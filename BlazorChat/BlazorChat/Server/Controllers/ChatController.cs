...
namespace BlazorChat.Server.Controllers
{
    ...
    public class ChatController : ControllerBase
    {
        ...
        [HttpGet("users/usernames")]
        public async Task<IActionResult> GetUsersNamesAsync()
        {
            var userId = User.Claims.Where(a => a.Type == ClaimTypes.Name).Select(a => a.Value).FirstOrDefault();
            var allUsers = await _context.Users.Where(user => user.UserName != userId).ToListAsync();
            return Ok(allUsers);
        }
